#!/usr/bin/env python3
"""
Universal Agent Auto-Loop Hook System with Command Logging
Enhanced version with comprehensive command and issue processing logging
...
from command_logger import CommandLogger, get_logger
...
class UniversalAgentAutoLoopHookWithLogging:
    """
    Universal Enhanced Agent Auto-Loop Hook with comprehensive logging
    Logs all commands, API calls, and issue processing events
    Works with any GitHub repository
    """
    ...
    def __init__(self, config: AgentConfig):
        ...
        log_dir = f"/tmp/claude-code-logs/agent-{config.agent_id}-{config.repo_name}"
        self.cmd_logger = CommandLogger(log_dir)
    ...
    def fetch_available_tasks(self) -> List[Dict[str, Any]]:
        """Fetch available tasks from GitHub with logging"""
        try:
            # Build label filter
            ...
            with self.cmd_logger.log_command("GH_API", cmd, {"labels": self.config.labels, "repo": self.repo_full_name}):
                result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
                
                if result.returncode == 0:
                    issues = json.loads(result.stdout)
                    
                    # Log successful API call result
                    cmd_id = self.cmd_logger.log_command_start(
                        "API_RESULT", 
                        "fetch_issues_success",
                        {"issue_count": len(issues), "repo": self.repo_full_name}
                    )
                    self.cmd_logger.log_command_complete(
                        cmd_id,
                        f"Fetched {len(issues)} issues from {self.repo_full_name}",
                        0
                    )
    ...
    def execute_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """Execute a single task autonomously with logging"""
        ...
        # Log issue processing start
        self.cmd_logger.log_issue_processing_start(
            task_id,
            task_title,
            "TASK_EXECUTION",
            {
        ...
        try:
            ...
            # Log issue processing complete
            self.cmd_logger.log_issue_processing_complete(
                task_id,
                "TASK_EXECUTION",
                {"status": execution_result['status'], "duration": duration, "repo": self.repo_full_name}
            )
            ...
            self.cmd_logger.log_issue_processing_error(
                task_id,
                "TASK_EXECUTION",
                str(e),
                {"agent": self.config.agent_id, "repo": self.repo_full_name}
            )
    ...
    def assign_task_to_agent(self, task_id: str):
        """Assign GitHub issue to the agent with logging"""
        try:
            cmd = f'gh issue edit {task_id} --repo {self.repo_full_name} --add-assignee {self.config.agent_id}'
            
            # Log command execution
            with self.cmd_logger.log_command("GH_API", cmd, {"task_id": task_id, "assignee": self.config.agent_id, "repo": self.repo_full_name}):
    ...
    def create_autonomous_session(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Log file creation
        with self.cmd_logger.log_command("FILE_WRITE", str(session_file), {"task_id": task_id, "repo": self.repo_full_name}):
...
- **Command Logging**: Enabled at /tmp/claude-code-logs/agent-{self.config.agent_id}-{self.config.repo_name}
...
## Auto-Commands (All commands are logged)
```bash
# Check task status
...
- All commands are logged to: /tmp/claude-code-logs/agent-{self.config.agent_id}-{self.config.repo_name}
- Database: command_history.db
- Log files: commands_*.log, issues_*.log
...
---
🤖 Autonomous Agent {self.config.agent_id} - Auto-Generated Task Execution for {self.repo_full_name} (with Logging)
"""
        return instruction
    
    def monitor_task_execution(self, task_id: str, start_time: datetime) -> Dict[str, Any]:
        """Monitor task execution and determine completion status with logging"""
        ...
        while True:
            ...
            if elapsed > max_duration:
                ...
                self.cmd_logger.log_issue_processing_error(
                    task_id,
                    "TASK_TIMEOUT",
                    f"Exceeded time limit of {max_duration}s",
                    {"elapsed": elapsed, "repo": self.repo_full_name}
                )
            ...
            if self.check_escalation_needed(task_id):
                ...
                self.cmd_logger.log_issue_processing_start(
                    ...
                    "",
                    "ESCALATION",
                    {"reason": "Escalation signal detected", "elapsed": elapsed, "repo": self.repo_full_name}
                )
    ...
    def check_task_completion(self, task_id: str) -> Dict[str, Any]:
        """Check if task is completed with logging"""
        try:
            cmd = f'gh issue view {task_id} --repo {self.repo_full_name} --json state,labels,comments'
            
            # Log API call
            cmd_id = self.cmd_logger.log_command_start("GH_API", cmd, {"task_id": task_id, "repo": self.repo_full_name})
            ...
            if result.returncode == 0:
                self.cmd_logger.log_command_complete(cmd_id, result.stdout[:200], duration_ms)
                ...
                self.cmd_logger.log_command_error(cmd_id, result.stderr, duration_ms)
                return {'completed': False}
    ...
    def check_escalation_needed(self, task_id: str) -> bool:
        """Check if task needs escalation with logging"""
        try:
            cmd = f'gh issue view {task_id} --repo {self.repo_full_name} --json comments'
            
            # Log API call
            with self.cmd_logger.log_command("GH_API", cmd, {"task_id": task_id, "repo": self.repo_full_name}):
                result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    ...
    def run_autonomous_loop(self, max_iterations: Optional[int] = None):
        """Main autonomous execution loop with logging"""
        self.logger.info(f"Starting autonomous loop for Agent {self.config.agent_id} on {self.repo_full_name}")
        
        # Log session start
        self.cmd_logger.log_issue_processing_start(
            "SESSION",
            f"Agent {self.config.agent_id} Session on {self.repo_full_name}",
            "SESSION_START",
            {"max_iterations": max_iterations, "config": self.config.__dict__}
        )
        ...
        self.cmd_logger.log_issue_processing_complete(
            "SESSION",
            "SESSION_END",
            {"iterations": iteration, "repo": self.repo_full_name}
        )
        
        # Export logs before exit
        export_path = self.cmd_logger.export_logs_to_json(
            f"/tmp/agent-{self.config.agent_id}-{self.config.repo_name}-session-export-{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        
        self.logger.info(f"Autonomous loop completed after {iteration} iterations")
        self.logger.info(f"Session logs exported to: {export_path}")
        
        # Print command statistics
        stats = self.cmd_logger.get_command_stats()
        self.logger.info("Command Statistics:")
        for cmd_type, cmd_stats in stats.items():
            self.logger.info(f"  {cmd_type}: {cmd_stats}")
...
def main():
    """Main entry point for universal agent system with logging"""
    import argparse
    
    parser = argparse.ArgumentParser(description='Universal Agent Auto-Loop Hook System with Logging')
    ...
    hook = UniversalAgentAutoLoopHookWithLogging(config)
